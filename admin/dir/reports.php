...
<?php
    ...
    {
        echo "<tr><td>".$row['inv_for']."</td><td>BDT. ".$row['amount_bdt']."</td><td>S$ ".round($row['amount_sgd'],2)."</td><td>S$ ".round($row['amount_usd'],2)."</td><td><strong>".$row['approved']."</strong></td><td><a target='_blank' href='process_bill.php?id=".$row['id']."'>".$row['processed']."</a></td><td>".$row['date']."</td><td><a target='_blank' href='view_bill.php?id=".$row['id']."' class='btn btn-success btn-xs'> <i class='far fa-eye'></i> View</a>  <a target='_blank' href='confirm_delete.php?id=".$row['id']."' class='btn btn-danger btn-xs'> <i class='far fa-trash-alt'></i> Delete</a></td></tr>";
    }
